//==============================================================
// SwipeManager This script is taken from a forum on Internet
...
namespace zombisoft
{
	...
	public class SwipeManager : MonoBehaviour
	{
		...
		[Tooltip("Ignore swipes over UI objects")]
		...
		void Update()
		{
			if (autoDetectSwipes)
			{
				if (ignoreUI) // Swipes over UI accepted
					DetectSwipe();

				else // UI block swipes
				{
					if (EventSystem.current.IsPointerOverGameObject()) // Swipe starts over UI
						return;
					else
						DetectSwipe();
				}
		...
		static void DetectSwipe()
		{
			...
			{
				float swipeAreaMin = Screen.height / 3.7f;
				...
				if (firstPressPos.y >= swipeAreaMin)
				{
